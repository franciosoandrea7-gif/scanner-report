...
if pin_tecnico:
    if TECNICI.get(tecnico_selezionato) == pin_tecnico:
        ...
        st.success(f"✍️ ID Tecnico Verificato. Il report verrà siglato a nome di: {tecnico_selezionato}")
    else:
        st.error("❌ PIN errato! Inserisci il codice corretto per abilitare la firma del tecnico.")
...
def invia_email_pdf(destinatario, allegato_path, nome_cliente):
    ...
    msg['To'] = destinatario
    msg['Cc'] = "franciosoandrea@me.com"  # Copia visibile nel client
    msg['Subject'] = f"Report Intervento - {nome_cliente}"
    msg.attach(MIMEText("Buongiorno, in allegato il report ufficiale Nova Servimpianti.", 'plain'))
    
    # Lista reale di spedizione per il server (Spedisce a ENTRAMBI gli indirizzi)
    ...
    try:
        ...
        st.warning(f"⚠️ Email non partita automaticamente: {e}")
...
def elabora_pdf(pdf_filename, data_str, cliente, email_cliente, cellulare_cliente, marchio, matricola, km, ore_lavoro, preventivo, urgente, guasto_segnalato, descrizione_lavori, file_immagine, stringa_firma, firma_tecnico):
    ...
    story.append(Paragraph(guasto_segnalato if guasto_segnalato else "N.D.", body_style if 'body_style' in locals() else b_style))
